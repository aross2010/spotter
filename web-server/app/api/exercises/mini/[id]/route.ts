  ...
  try {
    ...
    for (const row of exerciseHistory) {
      ...
      if (exercise.isUnilateral) {
        ...
        const intensityRpe =
          row.rpe ||
          (row.rir !== undefined ? RirToRpe.get(Number(row.rir)) : undefined)
        const intensityRir =
          row.rir ||
          (row.rpe !== undefined ? rpeToRir.get(Number(row.rpe)) : undefined)
