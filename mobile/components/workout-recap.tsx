...
const WorkoutRecap = (workout: Workout) => {
  ...
  const stats = [
    ...
    {
      label: 'Lifted',
      value: workout.exercises.reduce((acc, exercise) => {
        return (
          acc +
          exercise.sets.reduce((setAcc, exerciseSet) => {
            const setWeight = exerciseSet.weightLbs || exerciseSet.weightKg || 0
      ...
      {stats &&
        stats.map((stat, index) => {
          return (
            ...
            >
              <Txt twcn="text-xs font-poppinsLight text-light-textSecondary dark:text-dark-textSecondary">
                {stat.label}
              </Txt>
