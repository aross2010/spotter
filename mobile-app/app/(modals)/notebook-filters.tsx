...
const NotebookFilters = () => {
  ...
  const { applyTagFilters, currentNotebookEntries, tagFilters } = useNotebook() // use notebook entries to show the length, results will be on notebook page
  ...
  return loading ? (
    ...
    <SafeView>
      <View style={tw`flex-row justify-end mb-2`}>
        <Pressable
          hitSlop={12}
          onPress={() => console.log('toggle sort order')}
        >
          <CalendarArrowDown
            size={24}
